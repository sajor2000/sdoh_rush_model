#!/usr/bin/env python3
"""
Final balanced model training with proper calibration and TRIPOD figures
"""
...
def generate_tripod_figures(base_model, calibrated_model, X_test, y_test, feature_names, output_dir='results/figures'):
    ...
    fig.suptitle('SDOH Model Performance - Balanced Calibration', fontsize=16, fontweight='bold')
    ...
    # 2. Feature Importance (from base model)
    ...
    ax1.scatter([metrics['ppv'][optimal_idx]], [metrics['sensitivity'][optimal_idx]], 
    ...
    ax2.scatter([metrics['screening_rate'][optimal_idx]], [metrics['sensitivity'][optimal_idx]], 
    ...
    threshold_probs = np.linspace(0.01, 0.3, 50)
    net_benefits = []
    treat_all = []
    ...
    ax.axhline(y=0, color='k', linestyle='--', linewidth=1, label='Screen None')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'decision_curve_final.png'), dpi=300, bbox_inches='tight')
...
def main():
    ...
    # Generate figures
    optimal_threshold = generate_tripod_figures(base_model, calibrated_model, X_test, y_test, feature_cols)
    ...
    metadata = {
        ...
        'parameters': params,
        'feature_importance': dict(zip(feature_cols[:10], 
                                     base_model.feature_importances_[:10].tolist()))
    }
    
    with open('models/balanced_final_metadata.json', 'w') as f:
        json.dump(metadata, f, indent=4)
    
    print("\n✅ Complete! Model and TRIPOD-AI figures saved.")
    print(f"\n📊 Key Achievement: Balanced model with {screen_rate:.1%} screening rate")
